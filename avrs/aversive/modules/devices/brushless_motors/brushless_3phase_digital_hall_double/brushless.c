...
#if (BRUSHLESS_TYPE != BRUSHLESS_DIGITAL_DOUBLE)
#error mauvais fichier de config "brushless_double_config.h"
#endif
...
volatile uint8_t pwm_previous_sensors_1 = 0;     // previous state of sensors for PWM update 
...
if (--pwm_division_timer != 0)
  ...
  static int8_t angle_electrical_previous_1 = 0; // previous electrical angle
  static uint8_t angle_previous_sensors_0 = 0;     // previous state of sensors for angle calc
  static uint8_t angle_previous_sensors_1 = 0;     // previous state of sensors for angle calc
  ...
  static uint16_t pwm_update_timer_1 = 1;
    ...
    #ifdef BRUSHLESS_1_SENSORS_INVERT
      sensors_1  = (~ sensors_1) & 0x7;  // inversion of the sensors
    #endif
    ...
    #ifndef LOADTEST
    // skipping if no change
    if(sensors_1 != angle_previous_sensors_1)
    #endif
      {
      angle_previous_sensors_1 = sensors_1;


      angle_electrical = g_brushless_angle[sensors_1];      // calculate electrical angle  

      diff = angle_electrical - angle_electrical_previous_1;  // convert to angle delta
      angle_electrical_previous_1 = angle_electrical;         // store for next time
      ...
      #ifndef BRUSHLESS_1_INVERT
        diff *= -1;    // inversion of the angle reading
      #endif

      // update of the absolute angle with the delta
      //IRQ_LOCK(); // not necessary coz we enable ints only after
      g_brushless_1.position -= (brushless_position)diff * BRUSHLESS_POSITION_PRECISION;
      //IRQ_UNLOCK();

      }

/*********** END Motor 1 angle update  **********/
    }
  ...
  if(interrupt_pwm ==0)
    ...
    // frequency division state machine
    if (--pwm_update_timer_1 == 0)
      {
      uint8_t flags;
      IRQ_LOCK(flags);
      pwm_update_timer_1 = g_brushless_1_pwm_divider; // protected against glitches
      IRQ_UNLOCK(flags);
      // end of frequency division state machine
    
      #ifndef LOADTEST
      // skipping if same as last time
      if(sensors_1 != pwm_previous_sensors_1)
      #endif
        {
        brushless_torque torque;
        
        pwm_previous_sensors_1 = sensors_1;
      
      
        IRQ_LOCK(flags);
        torque = g_brushless_1_torque;
        IRQ_UNLOCK(flags);
      
      
      
      
        BRUSHLESS_1_PWM_SET_1( 
                              PWM_MAX/2 +                         // offset 50%
                              ((int8_t)pgm_read_byte(g_brushless_phase1 + sensors_1)) * // conversion from sensors
                              torque /*>> 1*/ );      // torque must be divided by 2. this is now done in the acess function
        BRUSHLESS_1_PWM_SET_2( 
                              PWM_MAX/2 +
                              ((int8_t)pgm_read_byte(g_brushless_phase2 + sensors_1)) * 
                              torque /*>> 1*/ );   
        BRUSHLESS_1_PWM_SET_3(  
                              PWM_MAX/2 +
                              ((int8_t)pgm_read_byte(g_brushless_phase3 + sensors_1)) *
                              torque /*>> 1*/ );
        }
      }
    /*********** END Motor 1 PWM update  **********/
    interrupt_pwm =0;
    }
...
void brushless_speed_update_manage(void * dummy)
{
	...
	IRQ_LOCK(flags);
	g_brushless_1.speed = g_brushless_1.position - g_brushless_1_position_previous;
	g_brushless_1_position_previous = g_brushless_1.position;
	IRQ_UNLOCK(flags);
...
brushless_speed speed_mem_1   = BRUSHLESS_MAX_SPEED;
brushless_torque torque_mem_1 = 0;
...
#ifdef BRUSHLESS_1_SENSORS_PULL_UP_RESISTORS
	sbi(BRUSHLESS_1_SENSOR_1_PORT,BRUSHLESS_1_SENSOR_1_BIT);
	sbi(BRUSHLESS_1_SENSOR_2_PORT,BRUSHLESS_1_SENSOR_2_BIT);
	sbi(BRUSHLESS_1_SENSOR_3_PORT,BRUSHLESS_1_SENSOR_3_BIT);
#endif
...
void brushless_1_set_parameters(brushless_speed speed, brushless_torque torque)
{
	...
	// inversion
#ifdef BRUSHLESS_1_INVERT
	torque *= -1;
#endif
  
	IRQ_LOCK(flags);
	g_brushless_1_pwm_divider  = pwm_divider;
	g_brushless_1_torque = torque;
	IRQ_UNLOCK(flags);
  
	pwm_previous_sensors_1 = 0; // force application of the torque
}
...
void brushless_1_set_position(brushless_position p)
{
	uint8_t flags;
	IRQ_LOCK(flags);
	g_brushless_1_position_previous += p - g_brushless_1.position; // avoids speed glitches  on pos change
	g_brushless_1.position = p;
	IRQ_UNLOCK(flags);
}
...
int32_t brushless_get_speed(void * motor_num)
{
	...
	if(motor_num)
		retour = brushless_1_get_mesures();
	else
...
{
	if(motor_num)
		brushless_1_set_parameters(speed_mem_1, (brushless_torque) torque);
	else
...
{
	if(motor_num)
		brushless_1_set_parameters((brushless_speed) speed, torque_mem_1);
	else
