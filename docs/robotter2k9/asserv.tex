...
Le module \verb$hrobot_manager$ est initialisé dans le fichier \verb$cs.c$
ligne 70 :
\begin{verbatim}
...
Ce filtre permet de gérer 2 capteurs différents : boussole et capteur ADNS
sur une seul et même grandeur :
l'azimut du robot.\\

Or les mesures ont montrés que la mesure de cet azimut par intégration de la
vitesse de rotation mesurée par le capteur ADNS montre un temps de réponse 
rapide mais une importante dérive.\\

À l'inverse, la boussole, ne présente aucune dérive mais un temps de réponse
élevé et une résolution faible.\\

Le filtre est réglable par la valeur de $K_i$ gain intégral du filtre où pour
$K_i=0$ seul le capteur ADNS est pris en compte et pour $K_i\rightarrow1$ l'influence
de la boussole est maximum.\\

Une valeur de gain de $K_i=0.02$ est actuellement utilisé sur l'asservissement
du robot.

\subsection{Code}

Le filtre est implémenté dans les fichiers \verb$acfilter.c$ et
\verb$acfilter.h$.\\

Le filtre est initialisé à la ligne 66 du fichier \verb$cs.c$ :
\begin{verbatim}
...
La mise à jour de l'asservissement est effectuée par la fonction
\verb$robot_cs_update()$ appellée par \verb$cs_update()$, fonction appellée 
...
\item mise à jour de la boussole par appel à \verb$compass_update()$.
\item mise à jour de la position par appel à \verb$hposition_update()$.
