...
(Proportionnel,Dérivée) et une codeur incrémental HEDS5540 fixé à l'axe moteur.
...
dans l'espace du robot :
\begin{equation}
\left(\begin{array}{c} v_x\\v_y\\\omega_z\end{array}\right)_{robot}
\end{equation}

En une consigne en vitesse sur chaque moteur :
\begin{equation}
\left(\begin{array}{c}v_{M_1}\\v_{M_2}\\v_{M_3}\end{array}\right)_{moteurs}
\end{equation}
...
des unités n'a été effectuée $R$ étant égal à $1$ ici.
...
v_x\\v_y\\\omega_z\end{array}\right)$ la vitesse du robot, qu'il existe une matrice $A$
telle que :\\
...
L'élément $\left(\begin{array}{c}v_x\\v_y\\\omega_z\end{array}\right)$ calculé
précédement est donné dans le repère lié au capteur ADNS.\\
...
Le capteur de position constitué de 3 capteurs optiques ADNS6010 n'est pas
parfaitement ajusté dans les axes de la base roulante et il est, par conséquent,
difficile de calculer la matrice $A$ vue précédement a partir des mesures du
capteur.
...
La méthode est donc d'effectuer une série de mesure de couples $(U,V)$ où 
...
Il reste à effectuer un grand nombre de mesure sur des
coordonnées variées et d'ensuite identifier la matrice $A$ au moyen d'un
algorithme d'identification.

\section{Code}

L'identification est effectuée au moyen de la méthode ARMA sous SciLab
grâce au script\\\verb$unioc_asserv/scilab/adns_calibration.sci$.
